...
const ApplyButton = ({ href, label, size = 'lg' }: { href: string; label: string; size?: 'md' | 'lg' }) => (
  <a
    href={href}
    target="_blank"
...
export default async function RolePage({
  ...
  // JobPosting JSON-LD — exactly the structure required for Google for Jobs.
  const jobPostingLd = {
    ...
    <>
      ...
      <Breadcrumbs
        items={[
          ...
          { label: role.shortTitle, href: `/careers/${role.slug}` },
        ]}
      />
      ...
      <article>
        ...
        <header className="bg-gradient-to-br from-brand-950 via-brand-900 to-brand-800 px-4 py-16 text-white sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <div className="mx-auto max-w-4xl">
            ...
            <dl className="mt-6 grid gap-2 text-sm text-brand-200 sm:grid-cols-2">
              <div>
                <dt className="inline font-semibold text-white">Location: </dt>
                <dd className="inline">{role.location}</dd>
              </div>
              <div>
                <dt className="inline font-semibold text-white">Employment type: </dt>
                <dd className="inline">{role.employmentTypeDisplay}</dd>
              </div>
              <div>
                <dt className="inline font-semibold text-white">Department: </dt>
                <dd className="inline">{role.department}</dd>
              </div>
            </dl>
          </div>
        </header>
        ...
        <section className="border-b border-gray-200 bg-brand-50 px-4 py-10 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-4xl">
            <h2 className="text-xl font-bold text-brand-900">Compensation</h2>
            <ul className="mt-4 grid gap-3 sm:grid-cols-2">
              {role.compensation.map((item, i) => (
            ...
            <div className="mt-8 flex flex-col items-start gap-3 sm:flex-row sm:items-center">
              <ApplyButton href={role.applyUrl} label={role.applyButtonText} />
              <p className="text-sm text-brand-800">
                Applications go to Biri, Sales Development Manager.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <SectionHeading title="About the role" centered={false} />
            <p className="mt-6 text-base leading-relaxed text-gray-700">{role.about}</p>
          </div>
        </section>
        ...
        <section className="bg-gray-50 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <SectionHeading title="What you'll do" centered={false} />
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <SectionHeading title="The 7-day evaluation" centered={false} />
            <p className="mt-6 text-base leading-relaxed text-gray-700">
              {role.evaluation.intro}
            </p>
            <ul className="mt-4 space-y-2">
              {role.evaluation.targets.map((t, i) => (
                <li key={i} className="flex items-start gap-3 text-base text-gray-700">
                  <svg className="mt-1 h-4 w-4 shrink-0 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                  <span>{t}</span>
                </li>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-900">
                {role.evaluation.passDescription}
              </div>
              <div className="rounded-xl border border-gray-200 bg-white p-4 text-sm text-gray-700">
                {role.evaluation.fallShortDescription}
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-gray-50 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <SectionHeading title="Day-one materials" centered={false} />
            <ul className="mt-6 space-y-3">
              {role.dayOneMaterials.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-base text-gray-700">
                  <svg className="mt-1 h-4 w-4 shrink-0 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-5xl">
            <div className="grid gap-12 lg:grid-cols-2">
              <div>
                <SectionHeading title="You're a fit if" centered={false} />
                <ul className="mt-6 space-y-3">
                  {role.fitYes.map((item, i) => (
                    <li key={i} className="flex items-start gap-3 text-base text-gray-700">
                      <svg className="mt-1 h-4 w-4 shrink-0 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                      </svg>
              ...
              <div>
                <SectionHeading title="You're not a fit if" centered={false} />
                <ul className="mt-6 space-y-3">
                  {role.fitNo.map((item, i) => (
                    <li key={i} className="flex items-start gap-3 text-base text-gray-700">
                      <svg className="mt-1 h-4 w-4 shrink-0 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden="true">
        ...
        <section className="bg-gray-50 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <SectionHeading title="What we will never ask for" centered={false} />
            <ul className="mt-6 space-y-3">
              {role.neverAskFor.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-base text-gray-700">
                  <svg className="mt-1 h-4 w-4 shrink-0 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
            ...
            <p className="mt-6 text-sm text-gray-500">
              If anyone claiming to represent Iris Zimmerfrau Inc. asks you for money or
              sensitive banking information, please report it to{' '}
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl">
            <SectionHeading title="How to apply" centered={false} />
            <p className="mt-6 text-base leading-relaxed text-gray-700">
              {role.application.intro}
            </p>
            <ol className="mt-6 space-y-3">
              {role.application.steps.map((step, i) => (
                <li key={i} className="flex items-start gap-3 text-base text-gray-700">
